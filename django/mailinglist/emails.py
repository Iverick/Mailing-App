...
class EmailTemplateContext(Context):
    '''
    Creates context for a email subscription template
    '''
    @staticmethod
    def make_link(path):
...
def send_confirmation_email(subscriber):
    # function creates and sends confirmation emails to Subscribers
    mailing_list = subscriber.mailing_list
...
def send_subscriber_message(subscriber_message):
    # function renders into HTML and sends emails based on a SubscriberMessage
    ...
    if success == 1:
        # if send_mail() returned true it records the time the message was
        # sent
